...
program.command("export")
  ...
  .action(async (
    ...
    if (!output) {
      throw new Error("An output .pptx path is required (-o, --output, or positional)");
    }
...
program.command("render-html")
  ...
  .action(async (
    ...
    const output = options.output || positionalOutput;
    if (!output) throw new Error("An HTML render output directory is required");
...
try {
  ...
  process.stderr.write(`${JSON.stringify(createErrorReport(error), null, 2)}\n`);
  process.exitCode = 1;
}
